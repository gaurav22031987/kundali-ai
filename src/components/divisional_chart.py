...
def map_house_contents(houses: tuple[House, ...], placements: Iterable[tuple], lang: str = "en") -> dict[int, dict[str, object]]:
    """Map existing planet-house assignments to visual cells without calculations."""
    planets = {number: [] for number in range(1, 13)}
    for placement in placements:
        name, house, *display = placement
        planets[house].append(display[0] if display else (PLANET_ABBREVIATIONS[name] if lang == "en" else planet_name(name, lang)))
...
def build_divisional_svg(houses: tuple[House, ...], placements: Iterable[tuple], lagna_label: str, accent: str, lang: str = "en") -> str:
    ...
    for number, (x, y) in HOUSE_CENTERS.items():
        ...
        planet_lines = cell["planets"] or ("-",)
        rendered_planets = "".join(f'<text x="{x}" y="{y + 24 + index * 14}" class="planets">{escape(label)}</text>' for index, label in enumerate(planet_lines))
        labels.append(f'<text x="{x}" y="{y - 32}" class="number">{number}</text><text x="{x}" y="{y - 14}" class="house">{escape(str(cell["sign"]))}</text>{lagna}{rendered_planets}')
...
def render_divisional_chart(houses: tuple[House, ...], placements: Iterable[tuple], lagna_label: str, accent: str = "#174b68", lang: str = "en") -> None:
